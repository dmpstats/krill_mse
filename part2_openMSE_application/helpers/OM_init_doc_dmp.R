...
#' =============================================================================
#'
#' Modified date: 09/11/2023 MSEtool release version: 3.7.0 Hacker: Bruno Caneco
#' (BC)
#'
#' Local version of script "OM_init_doc.r" with functions from {MSEtool} to
#' create OM Documentation
#'
#' These alterations were initially triggered to fix some apparent bugs in the
#' code, namely in `OMdoc()` and `writeSection()`. I wasn't able to figure out
#' the origin of the issue, as I didn't have issues when I first start using it
#' in mid-2022. I've checked the commits on the repo and nothing jumps out as a
#' potential cause. It deals with input parameters defining a range of values
#' (e.g. `Msd` and `h`), which make the code trip when calling if statements
#' with variable with length > 1. My suspicion is that issues are caused by some
#' code modification in other upper level functions.
#'
#' Secondly, the function `MSEtool::OMdoc` is also hacked to add the argument
#' `html_theme` to set-up the html theme of the rendered output, as well as
#' other options. The main motivation for the change was to avoid style clashes
#' when the rendered html is embedded in other documents.
#'
#'
#' NOTE 1: some of these changes were previously implemented in script
#' OMdoc_dmp.r, but creating this file for completeness with all the rquired
#' child functions
#'
#' #' IMPORTANT: sourcing this file will overwrite the corresponding functions
#' loaded with MSEtool. Still, this script is dependent on MSEtool
#'
#' =============================================================================
#' =============================================================================
...
Template <- function(type=c("Stock", "Fleet", "Obs", "Imp")) {
  type <- match.arg(type)
  if (type == "Stock") mat <-
      matrix(c("Mortality and age:  maxage, R0, M, Msd",
  ...
  for (X in Slots) {
    ...
    if (X != "Name" && X != "Source" && X!="Species" && X!="Common_Name" && X!="Latitude" && X!='Longitude' && X!='Misc') {
    ...
    for (rr in 1:nrow(ClTemp)) {
      if (grepl("^Currently unused", ClTemp[rr,1])) {
        ...
        for (sl in slots) {
          ...
          if (inc.descript) {
            des <- switch(class,
                          "Stock" = MSEtool::StockDescription,
                          "Fleet" = MSEtool::FleetDescription,
                          "Obs" = MSEtool::ObsDescription,
                          "Imp" = MSEtool::ImpDescription)
...
plotText <- function(OM, slots, RMDfile) {
  if (any(c("M", "h", "Linf", "L50", "D", "EffUpper", "qcv", "Vmaxlen", "DR") %in% slots)) {
    ...
    cat("\n### Obs Plots\n", append=TRUE, file=RMDfile, sep="")
    cat("```{r, echo=FALSE,include=FALSE}\n", append=TRUE, file=RMDfile, sep="")
    cat(paste0("input <- file.path(system.file(package = 'MSEtool'),'Rmd/Obs/Obs.Rmd')\n"), append=TRUE, file=RMDfile, sep="")
    cat(" out <- knitr::knit_child(input) \n", append=TRUE, file=RMDfile, sep="")
    cat("```\n\n", append=TRUE, file=RMDfile, sep="")
    ...
    cat("\n### Imp Plots\n", append=TRUE, file=RMDfile, sep="")
    cat("```{r, echo=FALSE,include=FALSE}\n", append=TRUE, file=RMDfile, sep="")
    cat(paste0("input <- file.path(system.file(package = 'MSEtool'),'Rmd/Imp/Imp.Rmd')\n"), append=TRUE, file=RMDfile, sep="")
    cat(" out <- knitr::knit_child(input) \n", append=TRUE, file=RMDfile, sep="")
    cat("```\n\n", append=TRUE, file=RMDfile, sep="")
...
plotDep <- function(OM, Pars=NULL, nsim=48, nyears=50, proyears=50, nsamp=3, col="darkgray",
                    breaks=10, lwd=2) {
  if (!methods::is(OM,"OM")) stop("Must supply object of class 'OM'")
...
plotSelect <- function(OM, Pars=NULL, pyears=4, sim=NA, type="l") {
  if (!methods::is(OM, "OM")) stop("Object must be class 'OM' ")
  nsim <- OM@nsim
