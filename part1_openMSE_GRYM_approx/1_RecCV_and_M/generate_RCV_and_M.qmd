...
subtitle: "Generating random draws of Recruitment CV and Natural Mortality under different Proportional Recruitment scenarios"
---
...
the Proportional Recruitment (PR) model, was firstly proposed by de @delamare94
...
PR framework also provides alternative methods to convey the uncertainty about
the true values of parameters $M$ and $CV_R$. These methods involve generating
...
projections at later stages of the analysis. The PR analysis performed here is
very similar to that applied in @maschette_etal21, with the exception that the
proportion of recruits is assumed to be log-Normally distributed (as opposed to
inverse-Beta).
...
For the purpose of this analysis, PR scenarios are defined by a distinct set of
...
before specifying the remaining shared parameters.
...
(@tbl-pr-scens) were provided in @maschette_etal21, with figures varying
depending on factors such as the choice of surveys, length frequency cut-off,
sample estimates weighting, etc. Further details on the estimation process
underlying each scenario can be found in that study.



```{r}
#| tbl-cap: "Recruitment proportion estimates under each scenario (from Maschette et al 2021)"
#| label: tbl-pr-scens
#| code-fold: true
...
We now define the choices for the other model parameters, which remain unchanged
across all the considered PR scenarios.
...
#| cache: TRUE
#| out-width: 100%
#| fig-cap: "Highest density regions of generated draws of $CV_R$ and $M$ under each scenario"
#| label: fig-hdr-M-RCV-draws
#| code-fold: true
...
population to fluctuate from year to year.
...
conveyed by the plots in @fig-hdr-M-RCV-draws and, relatedly, the standard
...
- draws generated under the "Initial values" scenario are notably more
condensed compared to the remaining scenarios. This indicates that the "Initial
values" scenario has the lowest uncertainty about both $M$ and $CV_R$ parameters.

- Despite the estimated low variance in recruitment, uncertainty about $M$ and
$CV_R$ under "Scenario 30" is larger than the "Initial values" scenario due to
recruitment proportion estimates being calculated from only 3 surveys (as
opposed to 17 under "Initial values")

- draws under "Scenario 6" and "Scenario 12" are similar, showing that
combining data from two surveys (@tbl-pr-scens) do not make an impact on the
estimation process.

- The way in which the proportion of recruits in each survey is determined,
such as how sample-level estimates are weighed and scaled ("Scenario 6" and
"Scenario 12" Vs. "Scenario 18" and "Scenario 24") or the chosen body length
cutoff point ("Scenario 18" Vs "Scenario 24"), determines the magnitude of the
estimates of $M$ and $CV_R$.
...
obtained from observed from surveys.


### Exemplify generation of recruitment series

For completeness, we simulate a recruitment series from a single draw of PR
...
convey the uncertainty about these parameters. These draws will be used as
...
current management rules in place for the krill stock.
