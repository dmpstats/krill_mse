...
here very similar to those applied in @maschette_etal21, with the exception that
the proportion of recruits is assumed to be log-Normally distributed (as opposed
to inverse-Beta).
...
pr_scenarios <- read_xlsx(
  ...
  select(-ref) |>
  mutate(n_surveys = as.integer(n_surveys))
```
...
pr_scenarios |>
  ...
  relocate(rec_scen_id, Maschete_description, rprop_calc_method, data_years, further_details, mn_Rprop:sd_Rprop) |>
  flextable() |>
  colformat_double() |>
...
pr_fixed_pars <- read_xlsx(
  ...
  filter(category %in% c("PR model", "Mortality", "Mortality", "Pop model structure", "Simulation options")) |>
...
n_steps <- as.integer(pr_fixed_pars$n_timesteps)
...
scens_sim_pars <- pr_scenarios |>
  select(rec_scen_id, mn_Rprop, sd_Rprop, n_surveys) |>
...
if(run_fitting){
  ...
  cvR_M_draws_scen <- scens_sim_pars |>
    rowwise() |>
    pmap(function(rec_scen_id, mn_Rprop, sd_Rprop, n_surveys, M0, CV0, ref_age, qdist, Msf){
      
      cli::cli_alert(glue::glue("Running {rec_scen_id}"))
      ...
      cli::cli_alert_success(glue::glue("Finished {rec_scen_id}"))
      out
    })
  toc()
  ...
  names(cvR_M_draws_scen) <- scens_sim_pars$rec_scen_id
...
```{r}
cvR_M_draws_scen$`PR-1` |> round(3) |> glimpse()
cvR_M_draws_scen$`PR-3` |> round(3) |> glimpse()
```
...
- draws generated under the "PR-1" scenario are notably more
condensed compared to the remaining scenarios. This indicates that "PR-1" has
the lowest uncertainty about both $M$ and $CV_R$ parameters.

- Despite the relatively low values of $CV_R$ drawn under "PR-3", uncertainty
about $M$ and $CV_R$ for this scenario is larger (i.e. SD is higher) than that
obtained under "PR-1". This is likely related to the fact that recruitment
proportions were calculated from only 3 surveys (as opposed to 17 under "PR-1").

- Differences in the distribution of draws of $M$ and $CV_R$ between scenarios
"PR-2" and "PR-4" illustrate the impact of length cutoff thresholds and sample
...
scens_sim_pars |>
  split(scens_sim_pars$rec_scen_id) |>
